let isTouchDevice = navigator.userAgent.match(/(iPhone|iPod|iPad|Android|BlackBerry|Windows Phone)/);
...
// Calcula Porcentagem
function calcPercByNumber(num, per) { return Math.round(((num / per) * 100)) +"%" };
function calcPercByPerc(num, per) { return (num/100) * per; }
...
Index.prototype.initContent = function() {
	...
	$('.opRadio').on('change', function() {
		...
		if ($prop) { $('#labelRadio' + $this).attr('aria-checked', 'true'); }
		
		$('input[name=optionsRadio]').attr('disabled', 'disabled');

		if ($(this).data('feed') != '') {
			$('#' + $(this).data('feed')).hide().removeClass('hidden').slideToggle(600);
		}
		
		$('#feed' + $val).hide().removeClass('hidden').slideToggle(600);
		$('html, body').animate({ scrollTop : $('#feed' + $val).offset().top - 100}, 800);
	});
}
...
Index.prototype.menu = function(){
	...
	function carregaData(data) {
		...
		$.each(_this.telas, function(i, val) {
			...
			if (val.titulo && _this.verTitulo != val.titulo) {
				if(val.unidade && _this.verTitulo != val.unidade) _unidade = 'Unidade: '; _this.unidades.push(val.titulo);
				$('.listaMenu').append('<li><a class="btnMenuLi" id="btnMenu'+i+'" href="' + val.url + '">'+val.titulo +'</a></li>');
    ...
    function selectActiveItem() {
		...
		if (namePage == '') { namePage = 'index.html'; }
		qualPage = jQuery.inArray(namePage, _this.paginas);
		qualPage = $.map(_this.menuInterno, function(obj, index) { $.map(obj, function(objInterno, i) { if (namePage == objInterno.url) _this.linkAtivo = [index, i]; }); });

		_template = _this.menuInterno[_this.linkAtivo[0]][_this.linkAtivo[1]].template;
		...
		$('.img-template').each(function(i, v){ 
			...
			arrayFile.pop();
			newSrc = arrayFile.join('/') + '/' + imgFile[0] + '_' + _template + '.' + imgFile[1];
		...
		$('.pageInfo').html('<span>' + unidadeInfo + '</span>Página ' + addZero(Number(_this.linkAtivo[1]) + 1) + ' de ' + totalInterno);
		$('.listaMenu').find('li').eq(_this.linkAtivo[0]).children('a').attr('href', 'javascript:void(0);').addClass('active');
	...
	$(window).bind('menuToggle', function(){
		...
		setTimeout(function(){ abreMenu(); }, abreFecha);
	});
	...
	function controlaNavegacao() {
		$(document).ajaxComplete(function(event, XMLHttpRequest, ajaxOptions) {
			...
			// CONTROLE POR BOTÕES
			$('.setaNavegacao[data-page="ant"]').bind("click", function() { navPages(prevPage.url); });
			$('.setaNavegacao[data-page="prox"]').bind("click", function() { navPages(nextPage.url); });
	...
	function navPages(tipoPagina) { window.location = tipoPagina; }
	...
	$('.wrapper').find('h1, h2, h3, h4, p, li, a').not('.visuallyhidden').attr('tabindex', '0').attr('role', 'text');
}
...
Index.prototype.preloadContent = function(type, data) {
	...
	$content.find("*").each(function(index, element) {
		...
		if(bg !== "" && bg.indexOf('linear-gradient') == -1 && bg !== "none" && bg.indexOf("svg+xml") == -1){
	...
	queue.on("complete", function(event){
		...
		$('.loadingContent').fadeOut(600, function() { $('body').addClass('animaConteudo').trigger('classChange'); });
	}, this);
...
Index.prototype.Exercicio = function() {
	...
	let valoresAntigos = [],
		valoresNovos = ['Selecione uma opção','Selecione uma opção','Selecione uma opção','Selecione uma opção','Selecione uma opção','Selecione uma opção']
	...
	function atvFAse1() {
		let wordsArr = ['Etapa 1','Etapa 2','Etapa 3','Etapa 4','Etapa 5','Etapa 6'];   
			selectVal = [];               

		$('.atividade').find('.selectItens').each(function(i, v){
			$select = $('<select>');
			$select.addClass('selectWords').attr('id','select_' + addZero(i));
			$option = $('<option value="" selected="selected">Selecione uma opção</option>');
		...
		function contSelect() {
			...
			let options = document.querySelectorAll(".custom-a11yselect-option button"), //lista todas as opcoes
				combosSelecionados = document.querySelectorAll(".custom-a11yselect-container .custom-a11yselect-btn"), //lista os combos selecionados
			...
			for (let i = 0; i < valoresNovos.length; i++) {
				...
				if(idSelecionado == i){
					...
					if(combosSelecionados[idSelecionado].innerText != 'Selecione uma opção'){
			...
			for (let b = 0; b < valoresNovos.length; b++) {
				if(valoresAntigos[b] != valoresNovos[b]){
					...
					$.each(dArrayI, function(i, v){
						textoSel = $(v).text();
						if(textoSel == valoresAntigos[b]) { AdicionaRemoveDisable(v); }
					});
			...
			$.each(dArray, function(i, v){
				textoSel = $(v).text();
				if(textoSel == TextoDoValorSelecionado ) { AdicionaRemoveDisable(v); }
			});

			$('.atividade').find('.selectWords').each(function(i, v) {
				if (v.value != '' ) {
					cont++;
					selectVal[i] = v.value;
		...
		$('.atividade').find('.botaoConfirmar').on('click', function(){
			optionsVal = [];
			optionsRow = [];
			...
			$('.atividade').find('.selectWords').each(function(i, v) {
				...
				$ul = $(this).parents('.atividadeRow').find('.quadroAtividade').html();
			...
			$(this).attr('disabled', 'disabled').addClass('hidden');
			$('.atividade').find('button').attr('disabled', 'disabled').addClass('disabled');
			$('#faixaFeed').removeClass('hidden').fadeIn(200);
			$('html, body').animate({ scrollTop : $('#faixaFeed').offset().top - 115 }, 800);
		});
	}
...
svgLoader.prototype.loadContent = function(obj, func) {
	...
	obj.each(function(i, v){
		...
		$.get(imgURL, function(data) {
			...
			$svg.removeClass('hidden').css('width', '100%');
